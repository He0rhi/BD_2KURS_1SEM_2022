...
USE UNIVER
select a.AUDITORIUM_NAME, b.AUDITORIUM_TYPENAME, a.AUDITORIUM_CAPACITY
		from AUDITORIUM a 
		Inner Join AUDITORIUM_TYPE b on b.AUDITORIUM_TYPE=a.AUDITORIUM_TYPE
...
exec sp_xml_preparedocument @h output, @xml; --дескриптор sp_xml_preparedocument
select * from openxml(@h, '/SUBJECTS/SUBJECT', 0)
	with(SUBJECT char(10), SUBJECT_NAME varchar(100), PULPIT char(20))    --with для указания структуры какой элемент куда закидывается

insert into SUBJECT select * from openxml(@h, '/SUBJECTS/SUBJECT', 0) --дескриптор,выражение XPATH, режим работы, каждый артибут = столбец
	with(SUBJECT char(10), SUBJECT_NAME varchar(100), PULPIT char(20)) --0, атрибутивная система преобразования
	--1, аналогично 0 + для необработанных столбцов применяется сопоставления на основе xml-документа
	--2, каждый элемент преобразовывается в слобец таблицы
select * from SUBJECT where PULPIT LIKE 'РПР'
